...
def main():
    ...
    gyro.reset_angle(angle=0)  # Get ready with Gyro
    # Acutal movement data
    get_to_cross()
    travel2()
...
def get_to_cross():
    ...
    while get_color(left_colorsensor) != Color.BLACK:  # move to line detection
        robot.drive(100, 0)
...
def travel2():
    ...
    linefollow.line_follow(
        length=240 - 30 + 20, speed=200, sensor="right", side="left", Ki=0.0000
    )
    gyro_stop()

    main_motor.run_time(
        speed=1600, time=2000, then=Stop.HOLD, wait=True
    )  # put hand out
    gyro_straight(distance=100, speed=-200, t_prime=0.5) 
    gyro_stop()
